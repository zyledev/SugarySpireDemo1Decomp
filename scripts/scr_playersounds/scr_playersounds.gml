function scr_playersounds() //scr_playersounds
{
    with (obj_player)
    {
        if (state == 62 && (!audio_is_playing(sound_dash1)) && grounded)
            audio_play_sound(sound_dash1, 1, false)
        else if (state != 62 || (!grounded) || move == (-xscale))
        ...
        if (((state == 83 && sprite_index != spr_player_crazyrun) || sprite_index == spr_player_machslideboost3) && (!audio_is_playing(sound_maxspeed)))
            audio_play_sound(sound_maxspeed, 1, false)
        else if ((state != 83 && sprite_index != spr_player_machslideboost3) || sprite_index == spr_player_crazyrun)
