with (obj_player)
{
    ...
    if (other.image_yscale == -1)
    {
        ...
        if ((!key_up) && state == (56 << 0) && (!(place_meeting(x, (y - 1), obj_destructibles))) && place_meeting(x, (y - 1), other.id) && (state == 51 || state == 56) && (!(place_meeting(x, (y - 1), obj_destructibles))) && place_meeting(x, (y - 1), other.id) && (!instance_exists(obj_fadeout)))
        {
            scr_sound(79)
            other.depth = -10
            obj_player.box = 1
